...
print(f"Searching Spotify for album: {query}")
...
for i, a in enumerate(albums):
    print(f"  [{i}] {a['artists'][0]['name']} - {a['name']} ({a['release_date'][:4]})")

choice = input("Pick album number: ").strip()
album = albums[int(choice)]
album_id = album["id"]
...
print(f"\nDownloading {len(tracks)} tracks from {artist_name} - {album_name}\n")
